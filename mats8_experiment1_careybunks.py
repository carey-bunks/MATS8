# This is the code for demonstrating the effect of having a second
# token embedding table.  The two tables are implemented in the
# GPTLanguageModel class shown in Figure 1.
...
print()
print("*"*28)
print("Two Embedding Tables Example")
print("*"*28)
print()
...
class GPTLanguageModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.token_embedding_table1 = nn.Embedding(vocab_size, n_embd)
        self.token_embedding_table2 = nn.Embedding(vocab_size, n_embd)
        self.position_embedding_table = nn.Embedding(block_size, n_embd)
        self.blocks = nn.Sequential(*[Block(n_embd, n_head=n_head) for _ in range(n_layer)])
        self.lm_head = nn.Linear(n_embd, vocab_size)
        self.lingual_weights = nn.Parameter(torch.ones(2))

    def forward(self, idx, targets=None):
        B, T = idx.shape
        
        # idx and targets are both (B,T) tensor of integers
        tok_emb1 = self.token_embedding_table1(idx)  # (Batch, Time, Channels)
        tok_emb2 = self.token_embedding_table2(idx)  # (Batch, Time, Channels)
        pos_emb = self.position_embedding_table(torch.arange(T, device=device))  # (Time, Channels)
        x = self.lingual_weights[0]*(tok_emb1+pos_emb) + self.lingual_weights[1]*(tok_emb2 + pos_emb)
...
plt.title(f'Using Two Token Embedding Tables', fontsize = 24)
